...
api.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # puoi restringere ai tuoi domini
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def update_position(request: Request):
    """Riceve posizione GPS da tracker.html"""
...
try:
    dati = get_posizioni_attuali()  # funzione DB: restituisce [ {lat, lon, autista, targa}, ... ]
    if dati:
